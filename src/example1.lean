...
-- run_cmd print_dag
meta def print_div : tactic unit := do c ← get_env, class_dag c tt >>=
  (λ d, trace $ d.minimal_vertices [ `linear_order, `linear_ordered_add_comm_group, `ordered_ring ])
...
| (pi tna binder_info.inst_implicit tty tbody) (lam na binder_info.inst_implicit ty body) (n) (s) := do
  ...
  -- generalising_trace n,
  -- acc is the main logic, that will be folded over the type and the bode=y
...
meta def print_gens (decl : declaration) : tactic (option string) :=
...
section examples
  ...
  lemma bad10 (G H : Type*) [monoid G] [monoid H] [fintype G] [fintype H] (h : G ≃* H) :
    fintype.card G = fintype.card H:=
fintype.card_congr h.to_equiv
  -- multiple tings
...
-- #print filter.unbounded_of_tendsto_at_top'
...
#print ring_hom.char_p_iff_char_p
run_cmd (do
  e ← get_env,
  d ← e.get `category_theory.category.comp_id,
  print_gens e d,
  skip)
set_option pp.all false

variables {α : Type} {c b a : α} [linear_ordered_add_comm_group α]
...
(λ i, linear_ordered_ring.to_ordered_ring α i) _inst_1
