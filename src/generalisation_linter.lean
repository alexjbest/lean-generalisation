...
meta def is_instance_chain : ℕ → expr → tactic bool := λ n e, do (
  (do guardb e.is_app,
   ...
   l ← e.get_app_args.mfoldl (λ ol arg, (λ n, (n || arg.is_var) && ol) <$> is_instance_chain n arg) tt,
   return tt
   ) <|> return ff)
meta def target (t : expr) : tactic name :=
do e ← get_env,
...
-- open native.rb_set
open native.rb_set
meta def get_instance_chains : ℕ → expr → tactic (native.rb_set name) → tactic (native.rb_set name) := λ n e l,
do boo ← is_instance_chain n e,
  if boo then insert <$> l <*> target e else match e with
    | (app a a_1) := union <$> get_instance_chains n a l <*> get_instance_chains n a_1 l
    | (lam var_name bi var_type body) := union <$> get_instance_chains n var_type l <*> get_instance_chains n body l
    | (pi var_name bi var_type body) := union <$> get_instance_chains n var_type l <*> get_instance_chains n body l
    | (elet var_name type assignment body) := union <$> (union <$> get_instance_chains n type l <*> get_instance_chains n body l) <*> get_instance_chains n assignment l
    | (var a) := l
    | (sort a) := l
    | (const a a_1) := l
    | (mvar unique pretty type) := l
    | (local_const unique pretty bi type) := l
    | (macro a a_1) := l
    end

lemma aa (G : Type) [add_comm_semigroup G] (x : G) : G := x + x
